...
const LoginPage = () => {
  ...
  const { getUserDetails,setUser } = useAppContext();
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      setLoading(true);
      // try {
      //   console.log('Login about to make the request')
      ...
      //     // Use Promise.all to ensure both the user details fetch and user context setting are done before navigating
      //     await Promise.all([
      //       getUserDetails(), // Fetch user details
      //       setUser(res?.data.user), // Set the user in context
      //     ]);

      //     // After both promises resolve, navigate to the homepage or dashboard
      //     navigate("/");
      //   }
